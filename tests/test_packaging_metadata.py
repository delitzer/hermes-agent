...
# Minimum non-vulnerable Starlette: 1.3.1 includes the 2026 fixes for URL
# authority poisoning, Windows UNC/NTLM exposure, HTTPEndpoint method dispatch,
# and ignored urlencoded-form limits. Starlette is a transitive dep (fastapi in
# [web]; sse-starlette/mcp in [mcp]/[computer-use]/[dev]) so we pin it directly
# in every extra that exposes a server surface and enforce the floor in both
...
def test_starlette_pinned_above_2026_security_floor_in_pyproject():
    """Every extra that declares Starlette must pin a patched (>=1.3.1) version.

    Regression guard for the 2026 Starlette advisory set. A future edit that
    drops the pin or pins a pre-1.3.1 version fails here instead of shipping
    known request-validation and resource-exhaustion flaws.
    """
    data = tomllib.loads((REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    ...
    for extra in ("web", "mcp", "computer-use", "dev"):
        assert extra in found, (
            f"[{extra}] no longer pins starlette directly — 2026 security "
    ...
    for extra, ver in found.items():
        assert _version_tuple(ver) >= _STARLETTE_SECURITY_FLOOR, (
            f"[{extra}] pins starlette=={ver}, below the 2026 security fix "
            f"floor {'.'.join(map(str, _STARLETTE_SECURITY_FLOOR))}"
        )


def test_locked_starlette_meets_2026_security_floor():
    ...
    resolved version is >= the 2026 security fix floor so a stale-lock
    ...
    for ver in versions:
        assert _version_tuple(ver) >= _STARLETTE_SECURITY_FLOOR, (
            f"uv.lock resolves starlette=={ver}, below the 2026 security fix "
